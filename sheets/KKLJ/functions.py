...
def update_worksheet(database: Database, workbook: Workbook, worksheet: Worksheet, property: Property, skipFirstRow: bool = False) -> Worksheet:
    """
    ...
    Parameters:
        ...
        property: The property object associated with the worksheet 
        skipFirstRow: Whether to skip the first row during the update
        
    Returns:
        The updated worksheet
    """
    worksheet.row.number = worksheet.row.firstDataRow - 1
    worksheet, lastRowNumber = _update_worksheet(database, worksheet, skipFirstRow=skipFirstRow)
    if lastRowNumber > 180:
        return create_new_sheet_for_property(workbook, worksheet, property, lastRowNumber)
        
    set_latest_date_border(worksheet)
    return False


def _update_worksheet(database: Database, worksheet: Worksheet, skipFirstRow: bool = False) -> Worksheet:
    """
