...
def update_KKLJ_properties_sheets(
    ...
    for owner, properties in ownerProperties.items():
        ...
        for property in properties:
            ...
            newSheetDates = update_worksheet(database, workbook, sheet, property, skipFirstRow=skipFirstRow)
            if newSheetDates:
                sublog(f'CREATED new sheet for property: {propertyName}. Will need to reupdate after.')
                newSheets.append({sheetName: newSheetDates})
    ...
    for newSheet in newSheets:
        for propertyName, sheetDates in newSheet.items():
            ...
            update_KKLJ_properties_sheets(start=sheetDates[0], propertyName=propertyName)
