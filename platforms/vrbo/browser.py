...
class BrowseVrbo(KLTBrowser):
    ...
    def reservations(
        ...
        guestNamesPropIds: dict | None = None, 
        ...
        Parameters:
            ...
            guestNamesPropIds: Optional dictionary mapping guest names to dates
        ...
        Returns:
            Self reference for method chaining
        """
        self._set(propIdsDates, guestNamesPropIds, start, end)
    ...
    def _set(
        ...
        Parameters:
            ...
            guestNamesPropIds: Dictionary mapping guest names to dates
        ...
        self._guestNamesPropIds = guestNamesPropIds
        self._total = 0

        if propIdsDates:
            self._total += propIdsDates['total']
       
        if guestNamesPropIds:
            self._total += guestNamesPropIds['total']

        if start or end:
            self._totalPreviousReservationsFound = 0
        return self
    ...
    def _is_out_of_range(
        ...
        if self._start or self._end:
            if self._start and arrivalDate < self._start:
                self._totalPreviousReservationsFound += 1
                return True
        ...
        if self._propIdsDates:
            if 'new' in self._propIdsDates and propertyId in self._propIdsDates['new']:
                if (arrivalDate, departureDate) in self._propIdsDates['new'][propertyId]:
                    return False
            
            if 'updated' in self._propIdsDates and propertyId in self._propIdsDates['updated']:
                if (arrivalDate, departureDate) in self._propIdsDates['updated'][propertyId]:
                    return False
        ...
        if self._guestNamesPropIds and guestName in self._guestNamesPropIds.keys():
            if propertyId == self._guestNamesPropIds[guestName]:
                return False
