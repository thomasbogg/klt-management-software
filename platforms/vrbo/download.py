...
@update
def update_from_vrbo(start: date | None = None, end: date | None = None, singleBooking: bool = False) -> str:
    """
    ...
    Parameters:
        ...
        singleBooking: Whether to update a single booking only (default False)
    Returns:
    ...
    elif start and end and singleBooking:
        bookings = get_vrbo_bookings(database, date=start)
        start = end = None
    
    propIdsDates, reader = get_prop_ids_dates(messages)
    guestNamesPropIds = get_guest_names_prop_ids(bookings)
    reservations = get_reservations(propIdsDates, guestNamesPropIds, start, end)    
    toUpdate = list()
...
def get_guest_names_prop_ids(bookings: list[Booking]) -> dict:
    """
    ...
    """
    guestNamesPropertyId = {'total': 0}
    if not bookings:
        return guestNamesPropertyId
    
    for booking in bookings:
        guestNamesPropertyId['total'] += 1
        guestNamesPropertyId[booking.guest.lastName] = booking.property.vrboId
    return guestNamesPropertyId


def get_reservations(
    propIdsDates: dict, 
    guestNamesPropertyId: dict, 
    ...
    Parameters:
        ...
        guestNamesPropertyId: Dictionary mapping guest names to dates
    ...
    """
    total = propIdsDates['total'] + guestNamesPropertyId['total']
    tries = 0
    while tries < 3:
        tries += 1
