...
def _set_clean_and_guest_type(form: GoogleForm) -> None:
    ...
    form.newChoiceQuestionItem(
        ...
        options=[
            GoogleForm.Option(
                            value='1. Yes, I am the owner of the property and this booking is for me!', 
                            goToSectionId=ARRIVAL_METHOD_SECTION),
            GoogleForm.Option(
                            value='2. No, I am the owner of the property but I have booked these dates for someone else.', 
                            goToSectionId=MEET_GREET_SECTION),
            GoogleForm.Option(
                            value='3. No, I am not the owner of the property.', 
                            goToSectionId=MEET_GREET_SECTION),
        ],
        required=True,
    )
...
def _set_meet_greet_section(form: GoogleForm) -> None:
    ...
    form.newPageBreakItem(
        ...
        title='MEET & GREET',
        description=
            'As you selected that this booking is for someone other than the owner, please let ' \
            'us know if someone from our team will need to be available on arrival day to hand over keys.'
    )
    ...
    form.newChoiceQuestionItem(
        ...
        options=[
            GoogleForm.Option(
                value=
                    '1. Yes. Someone from our team will need to do a meet and greet and ' \
                    'hand over the keys.'),
            GoogleForm.Option(
                value=
                    '2. No. The arriving party will already have keys so no meet and greet is ' \
                    'required.'),
        ],
        required=True,
    )
...
def _set_guest_details_section(form: GoogleForm) -> None:
    ...
    form.newPageBreakItem(
        ...
        title='GUEST DETAILS',
        description=
            f'For us to be able to coordinate successfully with the arriving party, we ' \
            'need contact details. Please provide what you can.'
    )
    ...
    form.newTextQuestionItem(
        ...
        description=
            '\nIf an email address is provided, we will contact the arriving party ' \
            'directly with arrival instructions as well as final day reminders. This ' \
            'will relieve anyone else of the trouble of communicating with them.'
            '\n\nThe arriving party will be contacted from thomas@algarvebeachapartments.com .',
    )
    form.newTextQuestionItem(
...
def _arrival_form_description() -> str:
    ...
    return (
        ...
        "if this booking is not for the owner and the arriving party requires a "
        "key handover, we ask that an email address and phone number be provided for "
        "optimal coordination. Otherwise, you will not be asked to fill in this "
        "particular section and be directed straight to the arrival day plans.\n\n"
